...
# --------- CLEAN OLD ALIASES ---------
unalias please pls fuck 2>/dev/null
unalias ~ - md rd lg 2>/dev/null
unalias psg k9 pleaseupdate serve 2>/dev/null
unalias gco gcb gpl gcl gst glog 2>/dev/null
unalias matrix pleasework oops rage 2>/dev/null


# --------- BETTER ALIASES (now as functions) ---------


# sudo / last command magic
please() {
    sudo $(history -p !!)   # rerun last cmd with sudo after alias expansion
}
pls() { please "$@"; }
fuck() { please "$@"; }
...
home() { cd ~; }          # instead of ~()
back() { cd -; }          # instead of -()
md() { mkdir -p "$@"; }            # make dir + parents
rd() { rmdir "$@"; }               # remove dir
lg() { ls | grep "$@"; }           # grep ls output

# System
psg() { ps aux | grep "$@"; }      # find processes
k9() { kill -9 "$@"; }             # kill process fast
pleaseupdate() { sudo apt update && sudo apt upgrade -y && sudo apt autoremove -y; }
serve() { python3 -m http.server "${1:-8000}"; }  # default port 8000
...
# Fun / Troll
matrix() { cmatrix -C green; }
pleasework() { echo "🥺👉👈 please work" | lolcat; }
